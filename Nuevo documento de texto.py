...
def main():
    ...
    mqttc.subscribe("wlara123@outlook.es/repato",0)
    ...
    while(1):
        ...
        if(GPIO.input(boton1)==1):
            ...
            mqttc.publish("wlara123@outlook.es/repato2","1:0:-")
            GPIO.output(LED1,1)
        ...
        if(GPIO.input(boton2)==1):
            ...
            Arch.close()
            mqttc.publish("wlara123@outlook.es/repato2","0:1:-")
            GPIO.output(LED2,1)
            time.sleep(1)
            cntInfo=1
        if(cntInfo==1):
            Arch = open("DGuardados.dat","r")
            mqttc.publish("wlara123@outlook.es/repato2","0:0"+":"+Arch.read())
            Arch.close()
            time.sleep(1)
        cntInfo=0
